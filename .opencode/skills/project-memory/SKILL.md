...
- **⭐⭐ 第 21 次讨论（用户点名：反馈表单必须涵盖 5 项，已全部实现并实测）**：
  ...
  2. **前端**：`index.html` 在「名字或昵称」后新增两个 `.fb-field` → `<select class="fb-select" id="fbRelation" name="relation">`（请选择…/同学/朋友/家人/老师/网友/其他）与 `<select class="fb-select" id="fbDevice" name="device">`（请选择…/手机/平板/电脑/其他设备）；标签文案由「怎么称呼你」→「名字或昵称」，「想说的话」→「反馈内容」。
